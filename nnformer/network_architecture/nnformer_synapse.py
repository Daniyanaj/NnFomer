...
class MEDIUMVIT(SegmentationNetwork):
    def __init__(self, in_channels=4, out_channels=4, img_size=[128, 128, 128], norm_name='instance', window_size=32):
        super(MEDIUMVIT, self).__init__()
        ...
        if self.pos_embed not in ["conv", "perceptron"]:
            raise KeyError(f"Position embedding layer of type {self.pos_embed} is not supported.")
        ...
        self.encoder2 = UnetrPrUpBlock(
            ...
            out_channels=int(self.feature_size * 2),
            num_layer=1,
            kernel_size=3,
            stride=1,
        ...
        self.encoder3 = UnetrPrUpBlock(
            spatial_dims=3,
            in_channels=self.hidden_size,
            out_channels=int(self.feature_size * 4),
            num_layer=0,
            kernel_size=3,
            stride=1,
            upsample_kernel_size=2,
        ...
        self.encoder5 = UnetrPrDownBlock(
            spatial_dims=3,
            in_channels=self.hidden_size,
            out_channels=int(self.feature_size * 16),
            num_layer=0,
            kernel_size=3,
            stride=1,
        ...
        self.encoder6 = UnetrPrDownBlock(
            spatial_dims=3,
            in_channels=self.hidden_size,
            out_channels=int(self.feature_size * 32),
            num_layer=1,
            kernel_size=3,
            stride=1,
            downsample_kernel_size=2,
        ...
        self.out = [UnetOutBlock(spatial_dims=3, in_channels=self.feature_size * 2 ** i, out_channels=self.out_channels) for i in
                    range(5)]
        ...
        self.discriminator = PatchGAN(int(int(self.feature_size * 32)+1+4))
    ...
    def forward_features_fun(self,x_in):
    ...
    def forward(self, x_input):
        if self.training:
            ...
            try:                
                pred_discriminator = self.discriminator(x, logits, embeddings, modalities=True)
                target_discriminator = self.discriminator(x, target, embeddings, modalities=False)
                return logits, pred_discriminator, target_discriminator
            except:
