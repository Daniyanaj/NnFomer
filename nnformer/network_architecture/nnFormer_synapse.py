...
class HiLo(nn.Module):
    ...
    def hifi(self, x):
        B, H, W, S, C = x.shape
        h_group, w_group, s_group = H // self.ws, W // self.ws, S//self.ws
        total_groups = h_group * w_group* s_group

        x = x.reshape(B, h_group, self.ws, w_group, self.ws, s_group, self.ws,C).transpose(2, 3)
        ...
        attn = (attn @ v).transpose(2, 3).reshape(B, h_group, w_group, s_group, self.ws, self.ws, self.ws, self.h_dim)
        x = attn.transpose(2, 3).reshape(B, h_group * self.ws, w_group * self.ws, s_group*self.ws,self.h_dim)
        x = self.h_proj(x)
        return x
    ...
    def forward(self, x, H, W,S):
        ...
        x = x.reshape(B, H, W, S, C)

        # pad feature maps to multiples of window size
        pad_l = pad_t = pad_n= 0
        pad_r = (self.ws - W % self.ws) % self.ws
        pad_b = (self.ws - H % self.ws) % self.ws
        pad_g = (self.ws - S % self.ws) % self.ws
        x = F.pad(x, (0, 0, 0, pad_l, pad_r, pad_g, pad_t, pad_b))
        ...
        if pad_r > 0 or pad_b > 0 or pad_g>0:
            x = torch.cat((hifi_out[:, :H, :W, :S, :], lofi_out[:, :H, :W, :S,:]), dim=-1)
        else:
    ...
    def flops(self, N):
        H = int(N ** 0.5)
        Hp = Wp = Sp=self.ws * math.ceil(H / self.ws)
        ...
        nW = Np / self.ws / self.ws/self.ws
        window_len = self.ws * self.ws*self.ws
        # q @ k and attn @ v
        window_flops = window_len * window_len*window_len * self.h_dim * 2
        hifi_flops += nW * window_flops
        # projection
        hifi_flops += Np * self.h_dim * self.h_dim*self.h_dim
        ...
        # projection
        lofi_flops += Np * self.l_dim * self.l_dim* self.l_dim
...
class DTM(nn.Module):
    ...
    def __init__(self, input_resolution, dim, norm_layer=nn.LayerNorm):
        ...
        self.c_in = dim
        self.c_out = dim*2
        self.dconv = DeformConv3dPack(dim, dim*2, kernel_size=2, stride=2, padding=0)
        self.norm_layer = nn.BatchNorm3d(dim*2)
        self.act_layer = nn.GELU()

    def forward(self, x, H, W,S, return_offset=False):
        """
        x: B, H*W, C
        """
        B, L, C = x.shape
        x = x.reshape(B, H, W,S, C).permute(0, 3, 1, 2).contiguous()
        x, offset = self.dconv(x, return_offset=False)
        _, _, new_H, new_W ,new_S= x.shape
        x = self.act_layer(self.norm_layer(x)).flatten(2).transpose(1, 2)
        if return_offset:
            return x, new_H, new_W,new_S, offset
        else:
            return x, new_H, new_W,new_S
...
class BasicLayer(nn.Module):
    ...
    def forward(self, x, S, H, W):
        ...
        x = self.blocks[1](x,S,H,W)
        #x=self.blocks[3](x,S,H,W)
