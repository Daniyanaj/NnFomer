...
from vtunet.network_architecture.initialization import InitWeights_He
from vtunet.network_architecture.vtunet_tumor import VTUNet
from vtunet.training.data_augmentation.data_augmentation_moreDA import get_moreDA_augmentation
from vtunet.training.data_augmentation.default_data_augmentation import default_2D_augmentation_params, \
    get_patch_size, default_3D_augmentation_params
from vtunet.training.dataloading.dataset_loading import unpack_dataset
from vtunet.training.learning_rate.poly_lr import poly_lr
from vtunet.training.network_training.vtunetTrainer import vtunetTrainer
from vtunet.utilities.nd_softmax import softmax_helper
from vtunet.utilities.to_torch import maybe_to_torch, to_cuda


class vtunetTrainerV2_vtunet_tumor(vtunetTrainer):
    """
...
#            assert isinstance(self.network, (SegmentationNetwork, nn.DataParallel))
    ...
    def initialize_network(self):
        ...
        :return:
        """
        cfg = "./vtunet/configs/vt_unet_small.yaml"
        with open(cfg, 'r') as f:
            yaml_cfg = yaml.load(f, Loader=yaml.FullLoader)

        self.network = VTUNet(pretrain_ckpt=yaml_cfg.get("MODEL").get("PRETRAIN_CKPT"), num_classes=4,
                              embed_dim=yaml_cfg.get("MODEL").get("SWIN").get("EMBED_DIM"),
                              win_size=yaml_cfg.get("MODEL").get("SWIN").get("WINDOW_SIZE")).cuda()

        self.network.load_from(yaml_cfg.get("MODEL").get("PRETRAIN_CKPT"))
...
#         else:
#             print("Let's use single ", torch.cuda.device_count(), "GPUs!")
#             self.network.cuda()
#             
# =============================================================================
    ...
    def run_iteration(self, data_generator, do_backprop=True, run_online_evaluation=False):
        ...
        if torch.cuda.is_available():
            data = to_cuda(data)# data.to(self.device)#(data)
            target =to_cuda(target)# target.to(self.device)#to_cuda(target)
    ...
    def do_split(self):
        ...
        self.dataset_val2=self.dataset_val
